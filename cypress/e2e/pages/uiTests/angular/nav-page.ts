...
class NavPage extends BasePage {
    ...
    switch (menuItem) {
      ...
      case 'Products':
        ...
        if (
          ...
          cy.get(".dropdown-menu[aria-labelledby='navmen_" + products + "'] a[title='" + subMenuItem + "'] span")
            .contains(subMenuItem)
            .should('be.visible');
          cy.get(".dropdown-menu[aria-labelledby='navmen_" + products + "'] a[title='" + subMenuItem + "']", {
            timeout: 30000,
          }).within(() => {
            cy.root().then(link => {
      ...
      case 'Content & Events':
        //click on main header
        cy.findByRole('button', { name: 'Content & Events' }).should('be.visible').click();

        //check if subheader are below categary because they are external links
        if (subMenuItem == 'Global Practice Guides' || subMenuItem == 'Diversity and Inclusion') {
          cy.get(".dropdown-menu[aria-labelledby='navmen_" + articleEvents + "'] a[title='" + subMenuItem + "'] span")
            .contains(subMenuItem)
            .should('be.visible');
          cy.get(".dropdown-menu[aria-labelledby='navmen_" + articleEvents + "'] a[title='" + subMenuItem + "']", {
            timeout: 30000,
          }).within(() => {
            cy.root().then(link => {
              cy.request(link.prop('href')).its('status').should('eq', 200);
  ...
  verifyBreadcrumbText(breadcrumbText) {
    if (breadcrumbText != 'NA') {
      ...
      cy.get('span.text-secondary', { timeout: 10000 }).should('be.visible').should('contain', breadcrumbText);
    }
  }
